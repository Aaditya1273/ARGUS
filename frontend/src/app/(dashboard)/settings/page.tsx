...
export default function SettingsPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch('https://argus-production-d368.up.railway.app/api/v1/argus/signoz/health').then(r => r.json()),
      fetch('https://argus-production-d368.up.railway.app/api/v1/argus/signoz/config').then(r => r.text()),
    ]).then(([h, cfg]) => {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto animate-fadeIn">
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500 mt-1">ARGUS configuration and SigNoz Cloud integration status.</p>
      </div>
      ...
      <div className="card p-6 mb-5">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">Observability backend — receives all ARGUS spans</p>
          </div>
          {!loading && (
      ...
      <div className="card p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-sm font-semibold text-gray-900">ARGUS Backend</h2>
        </div>
        <div className="grid grid-cols-2 gap-3">
          {[
            { l: 'Backend URL',     v: 'https://argus-production-d368.up.railway.app' },
            { l: 'Dashboard URL',   v: 'http://localhost:3000' },
            { l: 'MCP Endpoint',    v: 'https://argus-production-d368.up.railway.app/api/v1/mcp' },
            { l: 'OAuth Discovery', v: 'https://argus-production-d368.up.railway.app/.well-known/oauth-authorization-server' },
          ].map(s => (
