...
export default function ReplayPage() {
  ...
  const run = async () => {
    ...
    try {
      const tr = await fetch(`https://argus-xhgx.onrender.com/api/v1/argus/replay/${encodeURIComponent(traceId)}`)
      if (!tr.ok) { setErr('Trace not found. Run a session first.'); return }
      const ex = await fetch('https://argus-xhgx.onrender.com/api/v1/argus/replay/execute', {
