...
way to install and manage Argus. This guide moves an existing Docker Compose
...
> Setting up Argus for the first time? Skip migration and install Foundry
> directly: [Argus install docs][install-docs].
...
- An existing Argus deployment from `install.sh` or `deploy/` (Compose or
  Swarm).
...
```bash
curl -fsSL https://argus.io/foundry.sh | bash
```
...
> config it references). Argus no longer distributes these files, so this copy
...
spec:
  ...
  patches:
    - target: "deployment/compose.yaml"
      operations:
        - op: replace
          path: /volumes/argus-telemetrykeeper-0-data/name
          value: argus-zookeeper-1
        - op: replace
          path: /volumes/argus-telemetrystore-0-0-data/name
          value: argus-clickhouse
        - op: replace
          path: /volumes/argus-metastore-sqlite-0-data/name
          value: argus-sqlite
        - op: add
          path: /services/argus-telemetrykeeper-zookeeper-0/user
          value: root
```
...
spec:
  ...
  patches:
    - target: "deployment/compose.yaml"
      operations:
        - op: replace
          path: /volumes/argus-telemetrykeeper-0-data
          value:
            name: argus-zookeeper-1
        - op: replace
          path: /volumes/argus-telemetrystore-0-0-data
          value:
            name: argus-clickhouse
        - op: replace
          path: /volumes/argus-metastore-sqlite-0-data
          value:
            name: argus-sqlite
        - op: add
          path: /services/argus-telemetrykeeper-zookeeper-0/user
...
- [ ] Container images match your current deployment. Foundry generates with
  `latest` by default; if your Argus version was older than latest, check the
...
docker stack deploy -c compose.yaml argus  # Swarm
```

## Verify

- All Argus containers are running.
...
- [Patches][patches]
- [Argus documentation][argus-docs]

[foundry]: https://github.com/Argus/foundry
[install-docs]: https://argus.io/docs/install/
[compose-example]: https://github.com/Argus/foundry/tree/main/docs/examples/docker/compose
[patches]: https://github.com/Argus/foundry/blob/main/docs/concepts/patches.md
[custom-config]: https://github.com/Argus/foundry/blob/main/docs/concepts/moldings.md#custom-config-files
[env-vars]: https://github.com/Argus/foundry/blob/main/docs/reference/casting-file.md#molding-spec
[casting-ref]: https://github.com/Argus/foundry/blob/main/docs/reference/casting-file.md
[ch-config]: https://clickhouse.com/docs/operations/configuration-files
[upgrade-path]: https://argus.io/docs/operate/upgrade/
[slack]: https://argus.io/slack
[foundry-issues]: https://github.com/Argus/foundry/issues
[argus-docs]: https://argus.io/docs
